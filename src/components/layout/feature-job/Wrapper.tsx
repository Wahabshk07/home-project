import { getPublicJobs } from "@/lib/api/public-api";
...
export async function FeaturedJobs() {
  let jobs: PublicJob[] = [];
  try {
    const { items } = await getPublicJobs(1, 80);
    jobs = items;
  } catch {
    jobs = [];
  }
...
return (
  <section className="w-full py-4 md:py-4 mt-[80px] bg-transparent px-4 md:px-4 lg:px-6">
