...
function EmployerPackageProfileInner() {
  ...
  const [catalog, setCatalog] = useState<JobPackageRow[]>([]);
    ...
    async (background = false) => {
      ...
      try {
        const [co, cat] = await Promise.all([
          ...
          listJobPackagesCatalog(accessToken),
        ]);
        setCompany(co);
        setCatalog(cat);
  ...
  useEffect(() => {
    if (checkout !== "success" || !stripeSessionId || !accessToken) return;
    let cancelled = false;
    void (async () => {
      try {
      ...
      try {
        ...
        /* user can use Refresh */
      }
    })();
    return () => {
      cancelled = true;
    };
  }, [checkout, stripeSessionId, accessToken]);
  ...
  async function startCheckout(pkg: JobPackageRow) {
    ...
    try {
      ...
      const { url } = await createStripeCheckoutSession(accessToken, {
        packageId: pkg.id,
      ...
      if (e instanceof BackendRequestError && isStripeCheckoutUsdMinimumError(e.message)) {
        setError(stripeCheckoutMinimumFailureHelp(pkg));
      } else {
        setError(
          e instanceof BackendRequestError
            ? e.message
